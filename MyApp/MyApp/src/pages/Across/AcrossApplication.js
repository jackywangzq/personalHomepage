...
@inject('rootStore')
@observer
export default class AcrossApplication extends React.Component {
  ...
  async getNext() {
    try {
      const result = await Request.post(`oppo/createOppoNeed?orgId=${this.state.orgId}&userId=${this.state.userId}&org2Id=${this.state.org2Id}`);
      const {oppoNum, approverPerson, groupHandler,yunGuangHandler,source,busin} = result;
      if (approverPerson.length === 0) {
         Alert.alert(
          '您暂时无法进行跨省支撑申请',
          '',
          [
  ...
  searchClick =(type) => {
    ...
    this.props.navigation.navigate('acrossSearch',{
      ...
      acrossvalue:this.state.acrossvalue,
      choose:(item,type) => {
        if(type == 'manager') {
          that.setState({
            managerId:item.id,
  ...
  async _submit() {
    ...
    || workload === '' || approverId === '' || dealId === '') {
      ...
      const keywords = [...this.state.selectkeywords].filter(item => item[1]).map(item => {
      ...
      try {
        const result = await Request.post(`oppo/initiate`, req);
              ...
              >
                {
                  this.state.dealId !== '' ?
                    <Profile
                      ...
                      avatarName={this.state.dealer.find(item => item.id === this.state.dealId).name}
                      name={this.state.dealer.find(item => item.id === this.state.dealId).name}
                      describe={this.state.dealer.find(item => item.id === this.state.dealId).phone}
                    />
                  :
              ...
              >
                {
                  this.state.approverId !== '' ?
                    <Profile
                      ...
                      avatarName={this.state.approverPerson.find(item => item.id === this.state.approverId).name}
                      name={this.state.approverPerson.find(item => item.id === this.state.approverId).name}
                      describe={this.state.approverPerson.find(item => item.id === this.state.approverId).phone}
                    />
                  :
